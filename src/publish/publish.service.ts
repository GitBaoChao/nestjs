...
export class PublishService {
  async publish(
    ...
    if (mode === "comment") {
      reviews.forEach((review) => {
        const { newPath, oldPath, type, endLine, issueContent, issueHeader } =
          review;
        ...
        gitProvider.publishCommentToLine(
          newPath,
          oldPath,
          endLine,
      ...
      reviews.forEach((review) => {
        ...
        const extendedDiffFile = extendedDiffFiles.find(
          (item) => item.new_path === newPath
        );

        const diffCode = this.getDiffCode(
          extendedDiffFile,
          type,
          startLine,
          endLine
  ...
  getDiffCode(
    extendedDiffFile: Change | undefined,
    type: "new" | "old",
    startLine: number,
    endLine: number
  ) {
    ...
    if (extendedDiffFile) {
      ...
      const linesWithNumber =
        type === "new" ? newLinesWithNumber : oldLinesWithNumber;
  ...
  publishNotification(
    ...
    const { pushUrl, userName, projectName, sourceBranch, targetBranch } =
      publishParams;
    if (!pushUrl) {
      return;
    }
